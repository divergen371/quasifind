...
external readdir_batch : dir_handle -> string array -> string array -> (string * kind * int * int) array = "caml_readdir_batch"

(* Iterator for batch entries *)
let iter_batch ?(prefixes=[||]) ?(suffixes=[||]) (path : string) (f : string -> kind -> int -> int -> unit) =
...
let readdir path =
  let acc = ref [] in
  iter_batch path (fun name kind size mtime ->
