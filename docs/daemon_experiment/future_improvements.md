...
JavaDocに相当する OCaml 標準のドキュメントツール **`odoc`** の導入。

**方針**:

- **docstring (`(** ... \*)`) の採用**: 公開関数や型に対して `odoc` 形式のコメントを記述する。
- **.mli への集約**: ユーザー向けドキュメントを `.mli` ファイルに集約し、インターフェースと仕様を分離する。
- **自動生成**: `dune build @doc` を活用し、常に最新のHTMLドキュメントを参照可能にする。

**メリット**:

- ライブラリとしての利用方法が明確になる。
- 外部ツール（LSPなど）を通じてエディタ上で関数の説明が表示されるようになる。
- プロジェクトの「完成品」としての外観が整う。
...
- [x] VFS重複保存問題の修正
- [x] インターフェースファイル (.mli) の導入 (#38)
- [x] ドキュメント標準化 (odoc) (#39)
- [x] ユーザーフレンドリーなエラーメッセージ (#9)
- [x] エラーコードの標準化 (#10)
- [x] Result型ベースのエラーハンドリング (#13)
- [x] 各モジュールのエラー改善 (#14)
- [x] エラー回復ストラテジー (#15)
