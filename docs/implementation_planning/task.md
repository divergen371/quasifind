...
- [x] **Phase 6: コマンド実行機能 (-x / -X)** <!-- id: 50 -->
  - [x] CLI オプション追加 (`-x`, `-X`) <!-- id: 51 -->
  - [x] 各ファイル実行 (`-x`) の実装 (Eio.Process) <!-- id: 52 -->
  - [x] バッチ実行 (`-X`) の実装 (全件収集後に実行) <!-- id: 53 -->
  - [x] コマンドテンプレート置換 (`{}` 対応) <!-- id: 54 -->

- [x] **Phase 7: 履歴と検索機能 (History & Fuzzy Search)** <!-- id: 60 -->
  - [x] 履歴保存モジュール (`History`) の実装 (JSONL形式等) <!-- id: 61 -->
