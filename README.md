...
> **【免責事項: 個人の実験プロジェクト】**  
> 本プロジェクトは**完全な個人の趣味および技術実験を目的**として開発されているものです。ターミナル上で人間が手動で扱うには仕様が尖りすぎており、既存の `find` に慣れ親しんだ方には不親切な（互換性のない）仕様が多数含まれています。  
> 本ソフトウェアの使用によって生じたいかなる損害、データ損失、あるいはトラブル等についても、開発者は一切の責任を負いません。自己責任のもとでご利用・ご笑覧ください。

Quasi-find (findもどき)は、型付き DSL を用いてファイルシステムを検索する、OCaml + Zig 製のコマンドラインツールです。当初は Unix の `find` コマンドの代替を目指し開発がスタートしましたが、現在は主に「AI 時代に最適化された GUI ファイルマネージャーのバックエンド検索エンジン」としての役割に特化しつつある、実験的な側面が強い独自ツールとなっています。

> [!WARNING]  
> **macOS (Apple Silicon) 専用ツールです**  
> Quasifind は極限の速度を追求するため、macOS カーネル固有の `fsevents` や Cスタブ構造体の低レベル依存を多用しています。Linux や Windows (WSL) 等へのクロスプラットフォーム対応は技術的保守コストの観点から**放棄**しました（詳細は後述）。
...
### 🐧 なぜ Linux や Windows 対応を放棄したのか？

Quasifind は当初マルチプラットフォームでの動作を想定していましたが、以下の技術的困難さから **macOS への完全特化** へと舵を切りました。

1. **Cレイヤー (FFI) と `struct stat` のOS間格差**
   OCaml から C言語の世界へ、そして Zig のシステムコールへと処理を委譲（AST プッシュダウン等）する構成をとっています。この際、ファイルメタデータを格納する `struct stat` 構造体が macOS (`st_mtimespec`等) と Linux (`st_mtim`等) で異なり、Zig の `@cImport` がこれらの Cマクロの差異を上手く吸収できず「未定義プロパティ」等でビルドが崩壊する問題に直面しました。
2. **Zig リンカと libc ヘッダの依存問題**
   共有ライブラリ (`.so`) 生成時のスタブリンクにおいて、macOS では `-undefined dynamic_lookup` などの強力なフラグが使えますが、Linux では C標準ライブラリ群への静的・動的リンクのフォールバックが極めて複雑になり、ビルドチェーンの保守コストが跳ね上がりました。
3. **性能限界の追求**
   「汎用的でどこでも動くコード」を書くための泥臭い OS 判別分岐 `#if` を無数に書き散らすことは、「シンプルで保守しやすい爆速バックエンド」というコンセプトに反します。カーネルAPI を直叩きして限界性能を引き出す macOS (Apple Silicon) 専用のエンジンとして割り切ることが最適解だと判断しました。
...
> [!CAUTION]
> 前述の通り、Linux (Ubuntu等) および Windows (WSL含む) 環境でのビルドは **非対応 (Unsupported)** です。Cインタフェース（スタブ）の OS 別差異によるコンパイルエラーが発生するため、macOS 上でのみ使用・開発してください。
