...
| ------------------------- | --------------- | ---------------- | ------------------------------------ |
| **quasifind (Daemon)** 🚀 | **309ms**       | **22.1x**        | VFS常駐 + TSV IPC ストリーミング     |
| **quasifind (Parallel)**  | 1.28s           | 5.3x             | `getattrlistbulk` により `fd` と激突 |
| **fd** (Rust)             | 1.26s           | 5.4x             | 高速だが Daemon モードに大差         |
| **quasifind (DFS)**       | 5.41s           | 1.2x             | DFSでも高速化                        |
| **find** (Unix)           | 6.84s           | 1.0x             | 基準                                 |
...
9. **macOS VFS Lock Bypass & Context-Aware Stat (getattrlistbulk)** 🆕:
   macOS環境では、メタデータを伴う複数スレッドでの走査においてカーネル層のVFSロック競合が大きな足枷となります。Quasifindでは以下の **コンテキスト・アウェア（状況適応型）** なアプローチにより、OSの仕様限界をハックしています。

- **メタデータ (size, mtime) が必要な場合**: macOS専用の `getattrlistbulk` API を用いて、エントリ一覧の取得と `lstat` 相当のシステムコールを一括化することで競合を緩和し、数千回のオーバーヘッドを圧縮。
- **メタデータが不要な場合 (名前検索のみ)**: AST分析から `needs_stat` オプションをZig FFI層にプッシュダウンし、重い `getattrlistbulk` すらも回避して最も軽量な純粋 `readdir` のみにフォールバック。
  結果として、名前検索時のシステムコール負荷を根絶し、同条件下の `fd` と比較して **約1.6倍** の爆速性能 (750ms以下) を達成しました。

> [!NOTE]  
> **macOS での並列性能と VFS ロックについて**
> ファイルシステム走査ツールを macOS で複数スレッド実行 (`-j 8` 等) すると、OS カーネル内部で **VFS (Virtual File System) レイヤのロック競合 (Contention)** が発生し、各スレッドがカーネル内で順番待ち（System タイムの肥大化）を引き起こすという特有の仕様限界があります（Rust製 `fd` 等の極限最適化ツールでも同等の限界に直面します）。
>
> 以前の Quasifind では、この競合を防ぐために「macOSでは並列数を強制的に4までにキャップする」という苦肉の策をとっていましたが、今回導入したコンテキスト・アウェアなスキャンにより、重い VFSアクセスとなる時間と回数を極小に絞り込めるようになりました。
> このようなカーネル起因のオーバーヘッドを完全に抜き去るための究極の解として、常駐型デーモンモードが用意されています。

> [!NOTE]  
> **Linux 環境での並列性能について**  
> Linux 環境では、macOS とは異なりカーネルレベルでの VFS ロック競合が大幅に緩和されているため、並列オプション (`-j N`) はより素直にスケールします。CPU コア数に応じてスレッド数を増やすことで、**ほぼ線形に近い性能向上**が期待できます。これは、OCaml 5 の並列処理能力と Zig FFI による低レベル最適化が最大限に活かされる環境です。
