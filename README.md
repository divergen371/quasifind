...
OCaml 5 のマルチコア並列処理と低レベル最適化により、Rust 製の高速検索ツール `fd` を大幅に上回る速度を実現しています。
...
| ------------------------- | --------------- | ---------------- | -------------------------------- |
| **quasifind (Daemon)** 🚀 | **309ms**       | **21.7x**        | VFS常駐 + TSV IPC ストリーミング |
| **fd** (Rust)             | 1.18s           | 5.7x             | 高速だが Daemon モードに大差     |
| **quasifind (Parallel)**  | 1.20s           | 5.6x             | `fd` と同等                      |
| **quasifind (DFS)**       | 1.92s           | 3.5x             | シングルスレッドでも高速         |
...
6. **SIMD Aho-Corasick Content Search**:
   `content =~ /.../` や `content == "..."` によるファイル内容検索で、**ARM NEON SIMD** と **Aho-Corasick/KMP ハイブリッドアルゴリズム**を組み合わせた高速スキャンを実装しています。
...
ARM NEON SIMD + Aho-Corasick アルゴリズムにより `grep -rl` の約 **3.7倍高速**です。
