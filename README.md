...
9. **macOS `getattrlistbulk` (VFS Lock Bypass)** 🆕:
   macOS環境において、従来の `readdir` + `lstat` の組み合わせはカーネル内のVFS層での深刻なロック競合を引き起こしました。Quasifindでは macOS専用の `getattrlistbulk` API をZig FFI層で直接叩くことで、**ディレクトリ内のエントリ一覧とメタデータ (ファイルサイズ・更新日時・種別) を1回のシステムコールで一括取得**しています。
   これにより、システムコール起因のカーネルシステムタイムを約70%削減し、これまで到達不可能だった `fd` と同等以上の並列ディスクスキャン性能（約37万ファイルを1.2秒台で走査）を叩き出すことに成功しました。
...
>
> 以前の Quasifind では、この競合を防ぐために「macOSでは並列数を強制的に4までにキャップする」という苦肉の策をとっていましたが、今回導入した **`getattrlistbulk`** バルクスキャンによって、システムとVFSのやり取りの回数を数千分の一に低下させ、ロック競合を事実上「回避 (Bypass)」することに成功しました。結果としてスレッドキャップを撤廃し、フルコアでスケールする強烈な速度（`fd` を凌駕する水準）へ到達しました。
