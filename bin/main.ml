...
let run root_dir expr_str_opt max_depth follow_symlinks jobs help_short =
...
let cmd =
  ...
  Cmd.v info Term.(ret (const run $ root_dir $ expr_str $ max_depth $ follow_symlinks $ jobs $ help_short))
